"""LoRA-only fine-tuning entry point (Method A).
...
triplet sampling). The difference is purely in the YAML config:
    ...
    model.lora:                # injects LoRALinear into attn.qkv + attn.proj
        rank: <int>
        blocks: [<int>, ...]
    model.projector-type: identity   # no projector head; backbone L2-normalised
                                       features are used directly

Run::

    python scripts/train_lora_only.py scripts/configs/train_lora_only.yml
"""
