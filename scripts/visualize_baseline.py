...
def parse_args():
    ...
    parser.add_argument("--output", type=str, default="outputs/visualize_baseline")
...
def main():
    ...
    for index, (ref_img, query_img, gt_mask) in enumerate(loader):
        ...
        axes[3].set_title("Baseline Blend")
        ...
        for ax in axes:
            ax.axis("off")
        fig.suptitle(
            f"baseline=Dinov3 | layers={selected_layers} weights={selected_weights}",
            fontsize=10,
            y=1.02,
        )
        fig.tight_layout()
